...
class Crop:
    ...
    def __init__(self, base_path) -> None:        
        ...
        self.ref_focal = utils.readFocal_pil(self.ref_path)
    ...
    def crop_one_img(self, file_no, buffer=1.1, save=True, verbose=False) -> np.ndarray:
        ...
        focal_ratio = self.ref_focal / utils.readFocal_pil(file_path)
        ...
        if file_no == 1:
            cropped = utils.crop_fov(np.array(img_rgb), 1. / focal_ratio, buffer=1.)
        else:    
            cropped = utils.crop_fov(np.array(img_rgb), 1. / focal_ratio, buffer=buffer)
